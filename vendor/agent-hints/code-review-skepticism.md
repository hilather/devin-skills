> Origin: `~/git/agent-skills/knowledge/code-review-skepticism/README.md`. Cursor skill: `~/.cursor/skills/skeptic-code-review/SKILL.md`. Refresh: `sh scripts/refresh-vendor.sh`.
...
3. **Spawn a skeptic subagent** whose only job is to attack the implementation. Give it the diff, the change's stated intent, and the workspace path so it can read surrounding code — a diff hunk alone hides most bugs. It returns concrete findings classified as **blocking** or **non-blocking**.
